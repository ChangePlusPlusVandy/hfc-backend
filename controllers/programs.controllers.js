...
const getProgram = async (req, res) => {
    try {
        ...
        if (!req.query.id)
            ...
            programs = await Program.find(mongoose.Types.ObjectId(req.query.id)).populate("roster");
...
const editProgram = async (req, res) => {
    try {
        const programId = mongoose.Types.ObjectId(req.body._id.programID.trim());
        if (req.body.content.roster) {
            req.body.content.roster.forEach(e => mongoose.Types.ObjectId(e.trim()));
        }
        if (programId) {
